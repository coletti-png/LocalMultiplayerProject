...
namespace FronkonGames.Artistic.Sharpen
{
  ...
  public sealed partial class Sharpen
  {
    ...
    private static readonly Sharpen[] NoEffects = new Sharpen[0];
    ...
    /// <remarks> This function use Reflection, so it can be slow. </remarks>
    public static Sharpen Instance
    {
      get
      {
        ...
        if (pipelineAsset != null)
        {
          ...
          for (int i = 0; i < scriptableRendererData.rendererFeatures.Count; ++i)
          {
            if (scriptableRendererData.rendererFeatures[i] is Sharpen)
              return scriptableRendererData.rendererFeatures[i] as Sharpen;
          }
        }
    ...
    /// <remarks> This function use Reflection, so it can be slow. </remarks>
    private static Sharpen[] Instances
    {
      get
      {
          ...
          for (int i = 0; i < rendererDataList.Length; ++i)
          {
            if (rendererDataList[i] != null && rendererDataList[i].rendererFeatures.Count > 0)
              foreach (var feature in rendererDataList[i].rendererFeatures)
                if (feature is Sharpen)
                  effects.Add(feature as Sharpen);
          }

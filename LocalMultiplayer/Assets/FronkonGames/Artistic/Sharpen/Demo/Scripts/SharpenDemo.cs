...
#if UNITY_EDITOR
using UnityEditor;
[CustomEditor(typeof(SharpenDemo))]
public class SharpenDemoWarning : Editor
{
...
/// <summary> Artistic: Sharpen demo. </summary>
/// <remarks>
...
public class SharpenDemo : MonoBehaviour
{
  ...
  private void Awake()
  {
    if (Sharpen.IsInRenderFeatures() == false)
    {
    ...
    this.enabled = Sharpen.IsInRenderFeatures();
  }

  private void Start()
  {
    settings = Sharpen.Instance.settings;
    ResetEffect();
  }
  ...
  private void OnGUI()
  {
    ...
    GUILayout.BeginHorizontal();
    {
      GUILayout.BeginVertical("box", GUILayout.Width(450.0f), GUILayout.Height(Screen.height));
      {
        ...
        GUILayout.Label("SHARPEN DEMO", styleTitle);

        GUILayout.Space(space);

        settings.algorithm = EnumField("Algorithm", settings.algorithm);

        if (settings.algorithm == Sharpen.Algorithm.Luma)
        {
          settings.lumaPattern = EnumField("  Pattern", settings.lumaPattern);
          settings.sharpClamp = SliderField("  Limit", settings.sharpClamp);
        }

        GUILayout.Space(space);

        settings.sharpness = SliderField("Sharpness", settings.sharpness);
        settings.offsetBias = SliderField("Offset bias", settings.offsetBias, 0.1f, 6.0f);
        settings.vibrance = SliderField("Vibrance", settings.vibrance, 0.0f, 2.0f);

        GUILayout.Space(space);

        settings.debugView = ToggleField("Debug", settings.debugView);

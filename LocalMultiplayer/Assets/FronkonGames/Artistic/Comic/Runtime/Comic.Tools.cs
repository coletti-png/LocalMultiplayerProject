...
namespace FronkonGames.Artistic.Comic
{
  ...
  public sealed partial class Comic
  {
    ...
    private static readonly Comic[] NoEffects = new Comic[0];
        ...
        if (pipelineAsset != null)
        {
          ...
          for (int i = 0; i < scriptableRendererData.rendererFeatures.Count; ++i)
          {
            if (scriptableRendererData.rendererFeatures[i] is Comic)
              return scriptableRendererData.rendererFeatures[i] as Comic;
          }
        }
    ...
    /// <remarks> This function use Reflection, so it can be slow. </remarks>
    private static Comic[] Instances
    {
      get
      {

...
#if UNITY_VERSION >= 600000
struct ArtisticAttributes
{
  uint vertexID : SV_VertexID;
  UNITY_VERTEX_INPUT_INSTANCE_ID
};

struct ArtisticVaryings
{
...
ArtisticVaryings ArtisticVert(ArtisticAttributes input)
{
  ArtisticVaryings output;
  ...
  output.screenPos = _ScreenParams.xy * (screenPos.xy / screenPos.w) - _ScreenParams.xy * 0.5;
...
{
  const half separatorWidth = 6.0 * TEXEL_SIZE.x;

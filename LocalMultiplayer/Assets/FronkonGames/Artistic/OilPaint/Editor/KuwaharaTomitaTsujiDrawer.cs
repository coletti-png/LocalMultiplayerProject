...
{
  /// <summary> Tomita-Tsuji inspector. </summary>
  [CustomPropertyDrawer(typeof(TomitaTsuji.Settings))]
  public class TomitaTsujiDrawer : Drawer
  {
    private TomitaTsuji.Settings settings;

    protected override string EffectName => "Tomita-Tsuji";

    protected override void ResetValues() => settings?.ResetDefaultValues();

    protected override void InspectorGUI()
    {
      settings ??= GetSettings<TomitaTsuji.Settings>();
      ...
      settings.radius = Slider("Radius", "Size of the Tomita-Tsuji filter kernel [2, 20]. Default 10.", settings.radius, 2, 20, 10);

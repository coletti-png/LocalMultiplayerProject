...
namespace FronkonGames.Artistic.OilPaint
{
  ...
  ///------------------------------------------------------------------------------------------------------------------
  [DisallowMultipleRendererFeature("Oil Paint: Tomita-Tsuji")]
  public sealed class TomitaTsuji : ScriptableRendererFeature
  {
    ...
    public override void AddRenderPasses(ScriptableRenderer renderer, ref RenderingData renderingData)
    {
      ...
      if (material == null)
      {
        string shaderPath = $"Shaders/TomitaTsuji_URP";
    ...
    private static readonly TomitaTsuji[] NoEffects = new TomitaTsuji[0];
    ...
    /// <remarks> This function use Reflection, so it can be slow. </remarks>
    public static TomitaTsuji Instance
    {
      get
      {
        ...
        if (pipelineAsset != null)
        {
          ...
          {
            if (scriptableRendererData.rendererFeatures[i] is TomitaTsuji)
              return scriptableRendererData.rendererFeatures[i] as TomitaTsuji;
          }
        }
    ...
    /// <remarks> This function use Reflection, so it can be slow. </remarks>
    private static TomitaTsuji[] Instances
    {
      get
      {
          ...
          for (int i = 0; i < rendererDataList.Length; ++i)
          {
            if (rendererDataList[i] != null && rendererDataList[i].rendererFeatures.Count > 0)
              foreach (var feature in rendererDataList[i].rendererFeatures)
                if (feature is TomitaTsuji)
                  effects.Add(feature as TomitaTsuji);
          }

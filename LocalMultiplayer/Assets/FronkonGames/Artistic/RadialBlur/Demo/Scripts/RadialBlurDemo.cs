...
#if UNITY_EDITOR
using UnityEditor;
[CustomEditor(typeof(RadialBlurDemo))]
public class RadialBlurDemoWarning : Editor
{
...
/// <summary> Artistic: Radial Blur demo. </summary>
/// <remarks>
/// This code is designed for a simple demo, not for production environments.
/// </remarks>
public class RadialBlurDemo : MonoBehaviour
{
  ...
  private void Awake()
  {
    if (RadialBlur.IsInRenderFeatures() == false)
    {
    ...
    this.enabled = RadialBlur.IsInRenderFeatures();
  }

  private void Start()
  {
    settings = RadialBlur.Instance.settings;
    ResetEffect();
  }
  ...
  private void OnGUI()
  {
    ...
    GUILayout.BeginHorizontal();
    {
      GUILayout.BeginVertical("box", GUILayout.Width(450.0f), GUILayout.Height(Screen.height));
      {
        ...
        GUILayout.Label("RADIAL BLUR DEMO", styleTitle);

        GUILayout.Space(space);

        settings.intensity = SliderField("Intensity", settings.intensity);

        settings.samples = SliderField("Samples", settings.samples, 2, 20);
        settings.density = SliderField("Density", settings.density);
        settings.falloff = SliderField("Falloff", settings.falloff, 0.0f, 10.0f);
        settings.fishEye = SliderField("Fish eye", settings.fishEye, -2.0f, 2.0f);

...
namespace FronkonGames.Artistic.RadialBlur
{
  ...
  public sealed partial class RadialBlur
  {
    ...
    private static readonly RadialBlur[] NoEffects = new RadialBlur[0];
    ...
    public static RadialBlur Instance
    {
      get
      {
        ...
        if (pipelineAsset != null)
        {
          ...
          {
            if (scriptableRendererData.rendererFeatures[i] is RadialBlur)
              return scriptableRendererData.rendererFeatures[i] as RadialBlur;
          }
        }
    ...
    /// <remarks> This function use Reflection, so it can be slow. </remarks>
    private static RadialBlur[] Instances
    {
      get
      {
          ...
          for (int i = 0; i < rendererDataList.Length; ++i)
          {
            if (rendererDataList[i] != null && rendererDataList[i].rendererFeatures.Count > 0)
              foreach (var feature in rendererDataList[i].rendererFeatures)
                if (feature is RadialBlur)
                  effects.Add(feature as RadialBlur);
          }

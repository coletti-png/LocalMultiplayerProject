...
#if UNITY_EDITOR
using UnityEditor;
[CustomEditor(typeof(NeonDemo))]
public class NeonDemoWarning : Editor
{
...
/// <summary> Artistic: Neon demo. </summary>
/// <remarks>
/// This code is designed for a simple demo, not for production environments.
/// </remarks>
public class NeonDemo : MonoBehaviour
{
  ...
  private void Awake()
  {
    if (Neon.IsInRenderFeatures() == false)
    {
    ...
    this.enabled = Neon.IsInRenderFeatures();
  }

  private void Start()
  {
    settings = Neon.Instance.settings;
    ResetEffect();
  }
  ...
  private void OnGUI()
  {
    ...
    GUILayout.BeginHorizontal();
    {
      GUILayout.BeginVertical("box", GUILayout.Width(450.0f), GUILayout.Height(Screen.height));
      {
        ...
        GUILayout.Label("NEON DEMO", styleTitle);

        GUILayout.Space(space);

        settings.intensity = SliderField("Intensity", settings.intensity);

        settings.strength = SliderField("Strength", settings.strength);
        settings.radius = SliderField("Radius", settings.radius, 1, 20);
        settings.blend = EnumField("Blend", settings.blend);
        settings.fisheye = SliderField("Scale", settings.fisheye, 0.0f, 2.0f);
        settings.speed = SliderField("Speed", settings.speed, 0.0f, 5.0f);

...
namespace FronkonGames.Artistic.ColorIsolation
{
  ...
  public sealed partial class ColorIsolation
  {
    ...
    private static readonly ColorIsolation[] NoEffects = new ColorIsolation[0];
    ...
    /// <remarks> This function use Reflection, so it can be slow. </remarks>
    public static ColorIsolation Instance
    {
      get
      {
        ...
        if (pipelineAsset != null)
        {
          ...
          for (int i = 0; i < scriptableRendererData.rendererFeatures.Count; ++i)
          {
            if (scriptableRendererData.rendererFeatures[i] is ColorIsolation)
              return scriptableRendererData.rendererFeatures[i] as ColorIsolation;
          }
        }
    ...
    /// <remarks> This function use Reflection, so it can be slow. </remarks>
    private static ColorIsolation[] Instances
    {
      get
      {
        if (UniversalRenderPipeline.asset != null)
        {
          ...
          List<ColorIsolation> effects = new();
          for (int i = 0; i < rendererDataList.Length; ++i)
          {
            if (rendererDataList[i] != null && rendererDataList[i].rendererFeatures.Count > 0)
              foreach (var feature in rendererDataList[i].rendererFeatures)
                if (feature is ColorIsolation)
                  effects.Add(feature as ColorIsolation);
          }

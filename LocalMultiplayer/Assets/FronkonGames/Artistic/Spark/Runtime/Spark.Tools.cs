...
namespace FronkonGames.Artistic.Spark
{
  ...
  public sealed partial class Spark
  {
    ...
    private static readonly Spark[] NoEffects = new Spark[0];
    ...
    public static Spark Instance
    {
      get
      {
        ...
        if (pipelineAsset != null)
        {
          ...
          {
            if (scriptableRendererData.rendererFeatures[i] is Spark)
              return scriptableRendererData.rendererFeatures[i] as Spark;
          }
        }
    ...
    /// <remarks> This function use Reflection, so it can be slow. </remarks>
    private static Spark[] Instances
    {
      get
      {

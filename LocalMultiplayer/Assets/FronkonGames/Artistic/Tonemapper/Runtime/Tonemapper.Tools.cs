...
namespace FronkonGames.Artistic.Tonemapper
{
  ...
  public sealed partial class Tonemapper
  {
    ...
    private static readonly Tonemapper[] NoEffects = new Tonemapper[0];
    ...
    /// <remarks> This function use Reflection, so it can be slow. </remarks>
    public static Tonemapper Instance
    {
      get
      {
        ...
        if (pipelineAsset != null)
        {
          ...
          {
            if (scriptableRendererData.rendererFeatures[i] is Tonemapper)
              return scriptableRendererData.rendererFeatures[i] as Tonemapper;
          }
        }
    ...
    /// <remarks> This function use Reflection, so it can be slow. </remarks>
    private static Tonemapper[] Instances
    {
      get
      {
          ...
          for (int i = 0; i < rendererDataList.Length; ++i)
          {
            if (rendererDataList[i] != null && rendererDataList[i].rendererFeatures.Count > 0)
              foreach (var feature in rendererDataList[i].rendererFeatures)
                if (feature is Tonemapper)
                  effects.Add(feature as Tonemapper);
          }

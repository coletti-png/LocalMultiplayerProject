...
namespace FronkonGames.Artistic.Shockwave
{
  ...
  public sealed partial class Shockwave
  {
    ...
    private static readonly Shockwave[] NoEffects = new Shockwave[0];
    ...
    public static Shockwave Instance
    {
      get
      {
        ...
        if (pipelineAsset != null)
        {
          ...
          {
            if (scriptableRendererData.rendererFeatures[i] is Shockwave)
              return scriptableRendererData.rendererFeatures[i] as Shockwave;
          }
        }
    ...
    /// <remarks> This function use Reflection, so it can be slow. </remarks>
    private static Shockwave[] Instances
    {
      get
      {
          ...
          for (int i = 0; i < rendererDataList.Length; ++i)
          {
            if (rendererDataList[i] != null && rendererDataList[i].rendererFeatures.Count > 0)
              foreach (var feature in rendererDataList[i].rendererFeatures)
                if (feature is Shockwave)
                  effects.Add(feature as Shockwave);
          }

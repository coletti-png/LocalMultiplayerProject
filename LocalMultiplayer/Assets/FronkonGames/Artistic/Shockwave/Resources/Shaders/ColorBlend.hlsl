...
inline float3 BlendBurn(const float3 s, const float3 d)
{
  ...
  const float3 denom = max(s, Eps);
  const float3 result = 1.0 - (1.0 - d) / denom;
  const float3 useResult = step(Eps, s);
  return lerp(0.0, result, useResult);
}
...
{
  const float Eps = 1.0e-10;
  const float3 denom = max(d, Eps);
  const float3 divided = s / denom;
  const float3 useDivided = step(Eps, d);
  return lerp(s, divided, useDivided);
}
...
{
  const float Eps = 1.0e-10;
  const float3 denom = max(1.0 - s, Eps);
  const float3 dodged = d / denom;
  const float3 useDodged = step(s, 1.0 - Eps);
  return lerp(1.0, dodged, useDodged);
}
...
inline float3 BlendVividLight(float3 s, const float3 d)
{
  const float Eps = 1.0e-10;
  // Left branch: s < 0.5
  const float3 denomLeft = max(2.0 * s, Eps);
  const float3 leftDiv = 1.0 - (1.0 - d) / denomLeft;
  const float3 useLeftDiv = step(Eps, s);
  const float3 left = lerp(s, leftDiv, useLeftDiv);

  // Right branch: s >= 0.5
  const float3 denomRight = max(2.0 * (1.0 - s), Eps);
  const float3 rightDiv = d / denomRight;
  const float3 useRightDiv = step(s, 1.0 - Eps);
  const float3 right = lerp(s, rightDiv, useRightDiv);

  // Select branch per component
  const float3 useRight = step(0.5, s);
  return lerp(left, right, useRight);
}
...
inline float3 BlendLuminosity(const float3 s, const float3 d)
{
  ...
  {
    const float Eps = 1.0e-10;
    const float denom = max(sLum - minC, Eps);
    return sLum + ((c - sLum) * sLum) / denom;
  }
  else if (maxC > 1.0)
  {
    const float Eps = 1.0e-10;
    const float denom = max(maxC - sLum, Eps);
    return sLum + ((c - sLum) * (1.0 - sLum)) / denom;
  }

...
namespace FronkonGames.Artistic.OneBit
{
  ...
  public sealed partial class OneBit
  {
    ...
    private static readonly OneBit[] NoEffects = new OneBit[0];
        ...
        if (pipelineAsset != null)
        {
          ...
          for (int i = 0; i < scriptableRendererData.rendererFeatures.Count; ++i)
          {
            if (scriptableRendererData.rendererFeatures[i] is OneBit)
              return scriptableRendererData.rendererFeatures[i] as OneBit;
          }
        }
    ...
    /// <remarks> This function use Reflection, so it can be slow. </remarks>
    private static OneBit[] Instances
    {
      get
      {
          ...
          for (int i = 0; i < rendererDataList.Length; ++i)
          {
            if (rendererDataList[i] != null && rendererDataList[i].rendererFeatures.Count > 0)
              foreach (var feature in rendererDataList[i].rendererFeatures)
                if (feature is OneBit)
                  effects.Add(feature as OneBit);
          }

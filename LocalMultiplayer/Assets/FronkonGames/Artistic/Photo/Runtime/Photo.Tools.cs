...
namespace FronkonGames.Artistic.Photo
{
  ...
  public sealed partial class Photo
  {
    ...
    private static readonly Photo[] NoEffects = new Photo[0];
    ...
    public static Photo Instance
    {
      get
      {
        ...
        if (pipelineAsset != null)
        {
          ...
          {
            if (scriptableRendererData.rendererFeatures[i] is Photo)
              return scriptableRendererData.rendererFeatures[i] as Photo;
          }
        }
    ...
    /// <remarks> This function use Reflection, so it can be slow. </remarks>
    private static Photo[] Instances
    {
      get
      {
